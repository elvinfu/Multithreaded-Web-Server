...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  char abs_root_dir[PATH_MAX];
  char abs_test_file[PATH_MAX];

  // If any result from realpath is nullptr
  if (realpath(root_dir.c_str(), abs_root_dir) == nullptr ||
      realpath(test_file.c_str(), abs_test_file) == nullptr) {
    return false;
  }

  string abs_root_dir_str = abs_root_dir;
  string abs_test_file_str = abs_test_file;

  // Add forward slash to path if it doesn't contain it
  if (!abs_root_dir_str.empty() && abs_root_dir_str.back() != '/') {
    abs_root_dir_str += '/';
  }

  // Check if abs_test_file path starts with the abs_root_dir path
  return abs_test_file_str.find(abs_root_dir_str) == 0;
}
...
uint16_t GetRandPort() {
  ...
  portnum +=
    (static_cast<uint16_t>(rand())) % 5000;  // NOLINT(runtime/threadsafe_fn)
  return portnum;
}
